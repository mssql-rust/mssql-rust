...
#[test_on_runtimes]
async fn cyrillic_collations_should_work<S>(mut conn: mssql::Client<S>) -> Result<()>
where
    S: AsyncRead + AsyncWrite + Unpin + Send,
{
    conn.simple_query(
...
async fn application_name_should_be_set_correctly<S>(mut conn: mssql::Client<S>) -> Result<()>
where
    S: AsyncRead + AsyncWrite + Unpin + Send,
{
    let row = conn
