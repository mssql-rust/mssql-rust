...
static NAMED_INSTANCE_CONN_STR: Lazy<String> = Lazy::new(|| {
    let instance_name = env::var("MSSQL_TEST_INSTANCE").unwrap_or("MSSQLSERVER".to_owned());
...
fn connect_to_named_instance() -> Result<()> {
    ...
    async_std::task::block_on(async {
        let config = mssql::Config::from_ado_string(&NAMED_INSTANCE_CONN_STR)?;
