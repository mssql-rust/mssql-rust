...
use mssql::time::DateTime;
use mssql::xml::XmlData;
use mssql::{Column, ColumnData, ColumnType, TokenRow};
use uuid::Uuid;

#[cfg(feature = "tds73")]
use mssql::time::{Date, DateTime2, DateTimeOffset, Time};
...
/// Mimic the "send query results across the network as JSON" flow: build a
/// row out of columns + data, then verify the whole thing round-trips.
#[test]
