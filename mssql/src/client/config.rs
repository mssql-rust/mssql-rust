...
/// required for connecting to the database with a [`Client`](crate::Client).
/// It also provides the server address when connecting to a `TcpStream` via
/// the [`Config::get_addr`] method.
///
/// When using an [ADO.NET connection string], it can be
/// constructed using the [`Config::from_ado_string`] function.
///
