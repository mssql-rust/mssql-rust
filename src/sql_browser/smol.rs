...
use async_io::Timer;
use async_net::{resolve, SocketAddr, TcpStream, UdpSocket};
...
impl SqlBrowser for TcpStream {
    ...
    async fn connect_named(builder: &Config) -> crate::Result<Self> {
        ...
        if builder.multi_subnet_failover {
            // Race a connection attempt against every resolved address at
            // once, returning as soon as the first one succeeds. Intended
            // for SQL Server Always On availability groups, where the
            // listener can resolve to addresses on multiple subnets and
            // only one is actually reachable at a given time.
